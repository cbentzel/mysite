...
export default function Navbar() {
  ...
  return (
    <div className="tufte tufte-body sticky top-0 z-50 border-b border-[var(--tufte-rule)]">
      <div className="tufte-container">
        <div className="flex flex-row justify-between items-center py-4 gap-4">
          ...
          <div className="flex flex-row items-center gap-4 md:gap-6 overflow-x-auto">
            {navLinks.map(({ href, label }) => (
              <Link href={href} key={href}>
                <a
                  className="text-sm md:text-base whitespace-nowrap pb-0.5"
                  style={{
                    borderBottom: isActive(href)
                      ? "1px solid var(--tufte-accent)"
